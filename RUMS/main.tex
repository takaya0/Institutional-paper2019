...
\begin{Definition+}(Generative Adversarial Networks)\\
    ...
    また, $G\in\Hil_{1}$と一様分布(または正規分布)に従う確率変数$Z:\Omega\to\mathcal{Z}$に対し, $G(Z)$が従う確率分布を$\mathbb{P}_{G}$とする. $G\in\Hil_{G}$を
    Generatorと呼び, $D\in\Hil_{D}$をDiscrimatorと呼ぶ.
    $\Hil = \Hil_{D}\times\Hil_{G}$とした時, 機械学習空間$\MLsp$を敵対的生成ネットワークという. 
\end{Definition+}
\begin{Remark+}(Discrimatorの定義域について)\\
    GANの定義上の$\X$はただの部分空間ではなく, 以下の条件を満たすものとする.
    \begin{align*}
        \forall G\in\Hil_{G}, D(\X)\subset\text{supp}(p_{data})\cap\text{supp}(p_{G})
    \end{align*}
    ここで, $\text{supp}(f) \coloneqq\{x\in\X\mid f(x)\neq0\}$である.
\end{Remark+}
...
\begin{Proposition+}($\cite{GAN}$)\label{ODis}\\
    $\MLsp$をGANとし, $G\in\Hil_{G}$とする. この時, $\text{supp}(p_{data})\cup\text{supp}(p_{G})$上の最適Discrimatorは以下で与えられる.
    \begin{align*}
